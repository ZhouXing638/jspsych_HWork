_Go_NoGo范式是心理学中的一个经典范式，该范式可以通过被试（参与实验的个体）的按键反应有效地研究反应抑制能力。jsPsych是一套为心理学实验量身定制的Web前端开发工具箱，有效地简化了我们在使用js编写心理学实验时的工作量。本文将从以下思路，介绍如何使用jsPsych编写Go_NoGo实验范式。_
## Go_NoGo实验包含的主要部分
### 实验正式开始前
